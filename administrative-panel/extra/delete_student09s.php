...
<div id="container">
	...
	{
		$sql="DELETE FROM students09 WHERE Id=".$_REQUEST['Id']."";
		
		if(mysql_query($sql, $conn1))
		{
			$sql_updt="UPDATE tbl_studentspi SET
			IsEntered				=	0
			WHERE Id				=	".$_REQUEST['P1StudentId']."";
			$res_updt=mysql_query($sql_updt, $conn_sscreslt);
			
			$ins="INSERT INTO tbl_pislog SET
			ActivityType			=		'Deletion-Is',
			ActivityDescription		=		'Record of ".$_REQUEST['Name'].' - '.$_REQUEST['GroupId'].' - SSC-Is is deleted from DB'."',
			ActivityRefNo			=		'".$_REQUEST['ActivityRefNo']."',
			StudentId				=		".$_REQUEST['Id'].",
			EmployeeId				=		".$_SESSION['emp_id']."";
			$res=mysql_query($ins, $conn1);
			
			?><script>alert('Student Deleted Successfully.');location.replace('allstudents_delete09s.php');</script><?php
		}
		else
		{
			?><script>alert('Error in Query.');location.replace('delete_student09s.php?Id=<?php echo $_REQUEST['Id'];?>');</script><?php
		}
	}
	?>
	<?php
	$sql="SELECT Id, P1StudentId, Name, FatherName, GroupId FROM vwadmstudents09s WHERE Id=".$_REQUEST['Id']."";
				...
				<div class="widget_wrap">
					...
					<div class="widget_content">
						<form action="" method="post" class="form_container left_label" onSubmit="return check_submit_form();" enctype="multipart/form-data">
							<input name="P1StudentId" id="P1StudentId" type="hidden" value="<?php echo $row['P1StudentId'];?>"/>
										...
										<ul>
											...
											<div class="form_grid_12">
												<div class="form_input">
													...
													<button type="button" name="reset" class="btn_small btn_blue" onclick="location.replace('delete_student09s.php?Id=<?php echo $_REQUEST['Id'];?>')" tabindex="3"><span>Reset</span></button>
												</div>
											</div>
											</li>
										</ul>
