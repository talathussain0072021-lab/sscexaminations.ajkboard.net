...
<div id="container">
	...
	{
		$sql1="SELECT Id, BatchStatus, AdmStatus, RevStatus FROM vwadmbatches09 WHERE BatchNo='".$_REQUEST['BatchNo']."' AND BatchType=1 AND ExamId=".$ExamId."";
		$res1=mysql_query($sql1, $conn1);
		$row1=mysql_fetch_array($res1);
		$num_rows=mysql_num_rows($res1);
		
		$BatchStatus=$row1['BatchStatus'];
		if($row1['AdmStatus'] == 1){ $BatchStatus=6; }
		...
		if($num_rows > 0)
		{
			...
			?><script>alert('Batch Updated Successfully.');location.replace('rev_radmbatches_add09.php');</script><?php
		}
		else
		{ ?><script>alert('Batch not Found.');location.replace('rev_radmbatches_add09.php');</script><?php }
	}//if(isset($_REQUEST['submit1']))
	
	if(isset($_REQUEST['submit2']))
	{
		$sql1="SELECT Id FROM vwadmbatches09 WHERE BatchNo='".$_REQUEST['BatchNo']."' AND BatchType=1 AND ExamId=".$ExamId."";
		$res1=mysql_query($sql1, $conn1);
		$row1=mysql_fetch_array($res1);
		$num_rows=mysql_num_rows($res1);
...
function load_data()
{
	...
	({
		...
		{
			AppData.AdmBatches09=data.AdmBatches09;
		}
	});
}
function update_record()
{
	...
	var BatchInfo = $.grep(AppData.AdmBatches09, function (e) { return e.BatchNo == BatchNo && e.ExamId == ExamId; })[0];
