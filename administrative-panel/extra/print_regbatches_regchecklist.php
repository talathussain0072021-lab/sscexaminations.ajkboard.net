...
<tr>
	...
	<td style="font-size:14pt; text-align:center; width:70%; vertical-align:middle;"><b>REG. BATCHES REG. CHECK LIST REPORT '.$SessionName.'</b></td>
...
<tr>
	...
	<td style="background-color:#D1E2F2; width:10%;"><strong>Dist. Code</strong></td>
	...
	<td style="background-color:#D1E2F2; width:30%;"><strong>Remarks</strong></td>           	
</tr></table>','O');

		$SrNo=$first_number+1; $Counter=1; $PreviousDistCode='0'; $NewDistCode='0';
		$PreviousInstCode='0'; $NewInstCode='0'; $InstituteSum=0; $TotalSum=0;
		...
		$sql="SELECT Id, BatchNo, RegStatus, RevStatus, StdCount, BatchFee, ChallanNo, InstituteCode, InstituteDistrict FROM vwregbatches WHERE BatchStatus=1 AND SessionId=".$SessionId." AND RegStatus=".$_REQUEST['BatchStatus']." ORDER BY InstituteDistrict, InstituteCode, Id ASC limit {$first_number}, 2000";
		$res=mysql_query($sql, $conn1);
		while($row=mysql_fetch_array($res))
		{							
			if($row['RegStatus'] == 0){ $Status=''; }
			else if($row['RegStatus'] == 1){ $Status='Ok'; }
			else if($row['RegStatus'] == 2){ $Status='Not Ok'; }
			
			$NewDistCode=$row['InstituteDistrict']; $NewInstCode=$row['InstituteCode'];			
			if($Counter == 1)
			{ 
				$PreviousDistCode=$row['InstituteDistrict'];
				$PreviousInstCode=$row['InstituteCode']; 
			}			
			
			if($NewInstCode != $PreviousInstCode)
			{
				$PreviousInstCode=$NewInstCode;
				$html.='<tr>
          					<td colspan="4" style="text-align:right; font-weight:bold;">Institute Count: &nbsp; </td>
							<td style="text-align:center; font-weight:bold;">'.$InstituteSum.'</td>
							<td></td>
						</tr>';
				$InstituteSum=0;		
			}
			
			if($NewDistCode != $PreviousDistCode)
			{
				$PreviousDistCode=$NewDistCode;
				$html.='</table>
				<pagebreak />
				<table width="100%" border="1" style="font-family:sans-serif; align:center; float:left; text-align:center; margin-left:10px; margin-right:10px; border-collapse:collapse; font-size:10pt;">';
			}
											
			$html.='<tr>
						<td style="width:10%;">'.$SrNo.'</td>
						<td style="width:10%;">'.$row['InstituteDistrict'].'</td>
						<td style="width:10%;">'.$row['InstituteCode'].'</td>
						<td style="width:20%;">'.$row['BatchNo'].'</td>
						<td style="width:20%;">'.$row['StdCount'].'</td>
						<td style="width:30%;">'.$Status.'</td>	
					</tr>';
					$SrNo++; $Counter++; $InstituteSum+=$row['StdCount']; $TotalSum+=$row['StdCount'];
		}//while($row=mysql_fetch_array($res))
