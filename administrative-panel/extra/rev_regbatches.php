...
<div id="container">
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action']=='update-record')
	{
		...
		if($_REQUEST['BatchStatus'] == 1)
		{
			...
			{
				$sql1="SELECT Id, BatchStatus, RegStatus, RevStatus FROM vwregbatches WHERE Id=".$BatchesArray[$i]."";
		...
		?><script>alert('Batch Updated Successfully.');location.replace('rev_regbatches.php');</script><?php
		}//if($_REQUEST['BatchStatus'] == 1)
		else if($_REQUEST['BatchStatus'] == 2)
		{
			for($i=0; $i < sizeof($BatchesArray); $i++)
			{
				$sql1="SELECT Id FROM vwregbatches WHERE Id=".$BatchesArray[$i]."";
				$res1=mysql_query($sql1, $conn1);
				$row1=mysql_fetch_array($res1);
					...
					<div class="widget_content">
						...
						<div class="invoice_action_bar" style="float: right;">
                        	<div class="btn_30_blue">
                               	<a href="rev_regbatches_add.php"><span class="icon add_co"></span>
						...
						if(isset($_REQUEST['search']))
						{
							...
							$sql="SELECT Id, BatchNo, BatchStatus, RegStatus, RevStatus, BatchCounter, InstituteId, BatchFee, ChallanNo FROM vwregbatches WHERE BatchStatus=1 AND RevStatus=0 AND SessionId=".$SessionId." AND InstituteId=".$_REQUEST['InstituteId']." ORDER BY Id ASC";
						...
						{
							$SrNo=1; $BatchSum=0;
							$sql="SELECT Id, BatchNo, BatchStatus, RegStatus, RevStatus, BatchCounter, InstituteId, BatchFee, ChallanNo FROM vwregbatches WHERE BatchStatus=1 AND RevStatus=0 AND SessionId=".$SessionId." ORDER BY Id ASC";
							$res=mysql_query($sql, $conn1);
							while($row=mysql_fetch_array($res))
							{?>
							<tr>
...
function update_batchstatus(form)
{
	...
	else
	{
		...
		if(BatchCount > 0)
		{
			location.replace('rev_regbatches.php?BatchesArray='+BatchesArray+'&BatchStatus='+BatchStatus+'&action=update-record');
		}
