...
<?php
$sql="SELECT count(Id) as tstd_count FROM vwadmstudents12 WHERE Id >0";
                ...
                <!--<a href="voucher_print.php?id=<?php //echo $_REQUEST['id'];?>">
					...
					<div class="widget_content">
						...
						while($last_number < $max_number)
						{
							...
							if(($first_number+299) > $max_number)
							{ $last_number= $last_number+($max_number-$last_number); }
							else
							//{ $last_number=$first_number+149; }
							{ $last_number=$first_number+299; }
						?>
						<tr height="50px">
							<td>-> <a href="print_allpics_report12.php?InstituteId=<?php echo $_REQUEST['InstituteId'];?>&first_number=<?php echo $first_number;?>&&last_number=<?php echo $last_number;?>" target="_blank">PRINT RECORD FROM <?php echo $first_number.'-'.$last_number;?></a></td>
						</tr>
						<?php
						}
						?>
