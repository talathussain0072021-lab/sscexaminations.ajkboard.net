...
<div id="container">
	...
	if(isset($_REQUEST['action'])&&$_REQUEST['action']=='update-status')
	{
		...
		?><script>location.replace('ed_radmbatches09s.php?message=Data Updated Successfully.');</script><?php
	}
			...
			<div class="grid_12">
				<div class="widget_wrap">
					...
					<div class="widget_content">
						...
						$sql="SELECT Id, Dated, BatchNo, BatchStatus, AdmStatus, RevStatus, BatchCounter, EChallanNo, InstituteId, StdCount, ChallanNo FROM vwadmbatches09s WHERE BatchStatus=3 AND BatchType=1 AND ExamId=".$ExamId." ORDER BY Id ASC";
						$res=mysql_query($sql, $conn1);
...
{
	location.replace('ed_radmbatches09s.php?Id='+Id+'&InstituteId='+InstituteId+'&action=update-status');
}
function cancel_status(Id,InstituteId)
{
	location.replace('ed_radmbatches09s.php?Id='+Id+'&InstituteId='+InstituteId+'&action=cancel-status');
}
</script>