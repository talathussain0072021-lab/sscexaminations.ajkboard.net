...
<div id="container">
	...
	{
		$sql = mysql_query("CALL PIIRemRollNoAllotment(".$_REQUEST['dummy1'].")", $conn1);
		
		if($sql)
		{
			?><script>alert('RollNo allotted Successfully.');location.replace('gen_rollno10.php');</script><?php
		}
		else
		{
			?><script>alert('Error in Query.');location.replace('gen_rollno10.php');</script><?php
		}
	}//if(isset($_REQUEST['submit1']))
	if(isset($_REQUEST['submit2']))
	{
		$sql = mysql_query("CALL vwrollnoslip10TableCreation(".$_REQUEST['dummy2'].")", $conn1);
		
		if($sql)
		{
			?><script>alert('RollNoSlip generated Successfully.');location.replace('gen_rollno10.php');</script><?php
		}
		else
		{
			?><script>alert('Error in Query.');location.replace('gen_rollno10.php');</script><?php
	...
	?>
	<?php
	$sql="SELECT COUNT(*) AS TotalStudents, COUNT(CASE WHEN RollNo!=0 AND StdAdmStatus=1 THEN 1 END) AS WithRollNo, COUNT(CASE WHEN RollNo=0 AND StdAdmStatus=1 THEN 1 END) AS WithOutRollNo FROM vwadmstudents10";
			...
			<div class="grid_12">
				<div class="widget_wrap">
					...
					<div class="widget_content">
						...
						<?php
						$SrNo=1;
						$sql="SELECT `RollNo`, COUNT(`RollNo`) AS C FROM `admbatchstudents10` GROUP BY `RollNo` HAVING COUNT(`RollNo`)>1";
						$res=mysql_query($sql, $conn1);
