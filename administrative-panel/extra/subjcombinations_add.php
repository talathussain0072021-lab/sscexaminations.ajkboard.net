...
<div id="container">
	...
	{
		$sql1 = "SELECT * FROM subjectcombinations WHERE Name='".$_REQUEST['Name']."'";
		...
		if($no_row1 > 0) {
			...
			$sql2 = "SELECT Subject1Id, Subject2Id, Subject3Id, Subject4Id, Subject5Id, Subject6Id, Subject7Id, Subject8Id, Subject9Id FROM subjectcombinations";
			...
			//echo $exists;
			if (!$exists) {
				$ins="INSERT INTO subjectcombinations SET
				Name			=		'".$_REQUEST['Name']."',
				...
				if(mysql_query($ins, $conn1)) {
					$sql = mysql_query("CALL vwsubcombinations09TableCreation()", $conn1);
					$sql = mysql_query("CALL vwsubcombinations10TableCreation()", $conn1);
					?><script>location.replace('subjcombinations.php?message=Data Inserted Successfully.');</script><?php
				} else {
					?><script>alert('Error in Query.');location.replace('subjcombinations_add.php');</script><?php
				}
			} else {
				?><script>alert('This combination already exists.');location.replace('subjcombinations_add.php');</script><?php
			}
		}//else
	}
	?>
						...
						<form action="" method="post" class="form_container left_label" onSubmit="return check_submit_form();">
							<ul>
								...
								<div class="form_grid_12">
									<div class="form_input">
										<button type="submit" name="submit" value="submit" class="btn_small btn_blue" tabindex="25"><span>Submit</span></button>
