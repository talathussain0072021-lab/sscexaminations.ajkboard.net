...
<?php
$sql="SELECT count(Id) as tbatch_count FROM vwregbatches WHERE BatchStatus=1 AND SessionId=".$_REQUEST['SessionId']."";
if($_REQUEST['id'] == 1)
{ $sql.=" AND RegStatus=".$_REQUEST['BatchStatus']."";  }
else if($_REQUEST['id'] == 2)
{ $sql.=" AND RevStatus=".$_REQUEST['BatchStatus']."";  }
					...
					<div class="widget_content">
							<table height="200px" width="70%" style="margin-left:20px;" class="tablarge_property">
								...
								{ 
									...
									if(($first_number+1999) > $max_number)
									{ $last_number= $last_number+($max_number-$last_number); }
									else
									//{ $last_number=$first_number+149; }
									{ $last_number=$first_number+1999; }									
									?>
									<tr height="50px">
									<td align="left" colspan="4" style="font-weight:bolder; font-family:Verdana, Arial, Helvetica, sans-serif; font-size:12px;">-> <a href="#" onclick="parent.jQuery.colorbox.close();window.open('<?php if($_REQUEST['id']==1){?>print_regbatches_regchecklist.php<?php } else {?>print_regbatches_revchecklist.php<?php }?>?&BatchStatus=<?php echo $_REQUEST['BatchStatus'];?>&first_number=<?php echo $first_number;?>&&last_number=<?php echo $last_number;?>','_parent');">PRINT RECORD FROM <?php echo $first_number.'-'.$last_number;?></a></td>
									</tr>
								<?php	
								}
								?>
