...
<div id="container">
	...
	<?php
	$sql1="SELECT COUNT(*) AS TotalStudents, COUNT(CASE WHEN IsPrinted=1 THEN 1 END) AS PrintedSlips FROM vwrollnoslip09sc WHERE IsRegular=0";
					...
					<div class="widget_content">
						...
						<tr>
							<td colspan="6" align="left">
								...
								<a class="iframe" href="choose_std-rollnoslips09s.php?StdudentId=<?php echo $_REQUEST['StdudentId'];?>&&RegNo=<?php echo $_REQUEST['RegNo'];?>&&RollNo=<?php echo $_REQUEST['RollNo'];?>&&BatchSr=<?php echo $_REQUEST['BatchSr'];?>&&Name=<?php echo $_REQUEST['Name'];?>&&CentreCode=<?php echo $_REQUEST['CentreCode'];?>&&IsRegular=0"><span class="badge_style b_done">Print All RollNo Slips</span></a>
							</td>
						</tr>
						</table>
						</form>
						...
						{
							$SrNo=1;
							$sql="SELECT Id, RegNo, RollNo, Name, FatherName, Gender, ExamShift, CombinationName, BATCH_ID, IsPrinted, ACentreCode, ACentreName FROM vwrollnoslip09s WHERE IsRegular=0";
							...
							<tr>
								...
								<td class="center">
								<?php if(in_array('160901',$_SESSION['emp_user_rights'])){?>
								<span><a class="action-icons c-delete" href="delete_rollnoslips092A.php?Id=<?php echo $row['Id'];?>&Type=0" title="Delete">Delete</a></span>
								<?php }?>
								</td>
								<td class="center"><a href="print_student_allrollnoslips09s.php?Id=<?php echo $row['Id'];?>" target="_blank"><span class="badge_style b_medium">RollNo Slip</span></a></td>
							</tr>
							<?php
							$SrNo++;
							}
