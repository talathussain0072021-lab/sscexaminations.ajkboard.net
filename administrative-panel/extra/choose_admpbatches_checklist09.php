...
<?php
$sql="SELECT count(Id) as tbatch_count FROM vwadmbatches11 WHERE BatchStatus=1 AND BatchType=2 AND SessionId=".$_REQUEST['SessionId']."";
if($_REQUEST['id'] == 1)
{ $sql.=" AND AdmStatus=".$_REQUEST['BatchStatus']."";  }
					...
					<div class="widget_content">
							<table height="200px" width="70%" style="margin-left:20px;" class="tablarge_property">
								...
								{ 
									...
									?>
									<tr height="50px">
									<td align="left" colspan="4" style="font-weight:bolder; font-family:Verdana, Arial, Helvetica, sans-serif; font-size:12px;">-> <a href="#" onclick="parent.jQuery.colorbox.close();window.open('<?php if($_REQUEST['id']==1){?>print_admpbatches_admchecklist11.php<?php } else {?>print_admpbatches_revchecklist11.php<?php }?>?&BatchStatus=<?php echo $_REQUEST['BatchStatus'];?>&first_number=<?php echo $first_number;?>&&last_number=<?php echo $last_number;?>','_parent');">PRINT RECORD FROM <?php echo $first_number.'-'.$last_number;?></a></td>
									</tr>
								<?php	
								}
								?>
