...
<div id="container">
	...
	if (isset($_GET['action']) && $_GET['action'] == 'update-fee') {
		...
		if ($rowVer['cnt'] >= 1) {
			echo "<script>alert('ChallanFee is already paid.');location.replace('updtchlnfee_admstudents10.php');</script>"; exit;
		}
		...
		for ($i = 0; $i < count($ids); $i++) {
			...
			ActivityType			=		'ChallanFeeUpdation-II',
		...
		?><script>location.replace('updtchlnfee_admstudents10.php?message=Data Updated Successfully.');</script><?php
	}
	?>
				...
				<div class="widget_wrap">
					...
					<div class="widget_content">
						...
						if(isset($_REQUEST['search']))
						{
							...
							$sql="SELECT Id, Name, FatherName, BatchNo, BatchSr, StdAdmStatus, AdmissionFee FROM vwadmstudents10 WHERE ChallanNo=".$_REQUEST['ChallanNo']." ORDER BY Id ASC";
...
function validateUpdate(form)
{
    ...
    if (IdList.length === 0) {
		...
		location.replace(
			'updtchlnfee_admstudents10.php?ids=' + idsJSON + '&oldfees=' + oldfeesJSON + '&newfees=' + newfeesJSON + '&ChallanNo='+ ChallanNo +
