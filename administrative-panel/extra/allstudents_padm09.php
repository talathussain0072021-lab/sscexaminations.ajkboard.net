...
<div id="container">
	...
	<?php
	$sql1="SELECT Id FROM vwadmstudents09 WHERE IsRegular=0 AND SessionId=".$SessionId."";
	$res1=mysql_query($sql1, $conn1);
	$num_rows1=mysql_num_rows($res1);
	
	$sql2="SELECT Id FROM vwadmstudents09 WHERE IsRegular=0 AND SessionId=".$SessionId." AND BatchId is Not NULL";
	$res2=mysql_query($sql2, $conn1);
	$num_rows2=mysql_num_rows($res2);
	
	$sql3="SELECT Id FROM vwadmstudents09 WHERE IsRegular=0 AND SessionId=".$SessionId." AND BatchId is NULL";
			...
			<div class="grid_12">
				<div class="widget_wrap">
					...
					<div class="widget_content">
						...
						{
							$SrNo=1;
							$sql="SELECT Id, SSCRegNo, SSCBoard, Dated, Name, FatherName, DOB, Gender, IsSpecial, Domicile, AdmissionType, InstituteId, GroupName, CombinationName, BatchNo, BatchSr, StdAdmStatus, StdRevStatus, AdmissionFee, ChallanNo, ACentreCode, RegistrationNo FROM vwadmstudents09 WHERE IsRegular=0 AND SessionId=".$SessionId."";
							...
							<tr>
								...
								<td class="center">
								<?php
								if(in_array('100401',$_SESSION['emp_user_rights']) && $row['InstituteId']!=0){?>
									<span><a class="action-icons c-Delete" onClick="return confirm('Are you sure you want to delete this?');" href="deleteallstudents_padm09.php?Id=<?php echo $row['Id'];?>" title="Delete">Delete</a></span>
								<?php }?>
								</td>
							</tr>
							<?php
