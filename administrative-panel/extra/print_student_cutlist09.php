...
ob_start();
ob_clean();
$mpdf=new mPDF('','A4-L','','',10,10,20,-02,05,10);
...
<tr><td style="text-align:center; font-size:10pt; font-weight:bold;">( SSC-I '.strtoupper($ExamFullName).' CUT LIST )</td></tr>
</table></div> ','O');

$Counter=1; $Count=1; $PreviousCode='0'; $NewCode='0';
$sql="SELECT Id, Name, FatherName, PicURL, Gender, PostalAddress, GroupId, IsRegular, Sub5SmName, Sub6SmName, Sub7SmName, Sub8SmName, AdmBatchNo, AdmBatchSr, RollNo, ACentreName, ACentreCode, ExamShift FROM vwcutlist09 WHERE  RollNo!=0";
...
while($row=mysql_fetch_array($res))
{
	...
	if($Count == 1)
	{
		...
		$html.='<div style="width:100%; float:left;">
				<table width="100%" border="0" align="center" style="font-family:sans-serif; text-align:center; border-collapse:collapse; font-size:8.5pt;">
					<tr><td colspan="9" style="text-align:left; font-weight:bold;"> CENTRE : '.strtoupper($row['ACentreName']).'</td></tr>
					<tr>
						...
						<td style="width:20%; font-weight:bold;">Subjects</td>
	...
	{
		$PreviousCode=$NewCode; $Counter=1; 
		$html.='<pagebreak />
