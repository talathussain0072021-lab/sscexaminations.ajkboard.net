	...
	if(isset($_REQUEST['submit']))
	{
		if($_FILES['PicURL']['name'] != "" && $_REQUEST['newcheckbox'] == 1){
			...
			if($_REQUEST['IsRegular'] == 1)
			{
				...
				$sql1="SELECT MAX(substring(PicURL,9,10)) AS MaxSerial FROM students09 WHERE substring(PicURL,9,10) like '".$serial_code."%'";
				...
				$target_path_image1 = 'pics09s/'.$serial_no.'.jpg';
				$target_path_image2 = '../institution-panel/'.'pics09s/'.$serial_no.'.jpg';
			...
			if($_REQUEST['IsRegular'] == 1)
			{
				...
				if((strpos($_REQUEST['PicURL1'], 'pics09s/') !== false) && ($int_length == 12))
				{
			...
			else
			{
				...
				else
				{
					...
					$serial_part=$serial_part+1;
					$serial_no=$serial_code.str_pad($serial_part,4,'0',STR_PAD_LEFT);
					$target_path_image1 = 'pics092A/'.$serial_no.'.jpg';
					$target_path_image2 = '../institution-panel/'.'pics092A/'.$serial_no.'.jpg';
				}
		...
		if(mysql_query($sql, $conn1))
		{
			...
			ActivityType			=		'PicInfoUpdation-Is',
		...
		{
			?><script>alert('Error in Query.');location.replace('updtpic_student09s.php?Id=<?php echo $_REQUEST['Id'];?>');</script><?php
		}
	}
	?>
	<?php
	$sql="SELECT Id, Name, FatherName, PicURL, Domicile, IsRegular, StdInstituteCode FROM vwadmstudents09s WHERE Id=".$_REQUEST['Id']."";
									...
									<ul>
										...
										<div class="form_grid_12">
											<div class="form_input">
												...
												<button type="button" name="reset" class="btn_small btn_blue" onclick="location.replace('updtpic_student09s.php?Id=<?php echo $_REQUEST['Id'];?>')" tabindex="4"><span>Reset</span></button>
											</div>
										</div>
										</li>
									</ul>
