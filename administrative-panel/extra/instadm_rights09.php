	...
	if (isset($_POST['action']) && $_POST['action'] == 'update-record') {
		...
		for ($i = 0; $i < count($InstitutesArray); $i++) {
			$sql_q = "UPDATE institutes 
					  SET AdmRights09 = '" . mysql_real_escape_string($RightsArray[$i]) . "' 
					  WHERE Id = " . intval($InstitutesArray[$i]);
			mysql_query($sql_q, $conn1);
		}

		echo "<script>alert('Rights Updated Successfully.');location.replace('instadm_rights09.php');</script>";
	}
	?>
			...
			<div class="grid_12">
				<div class="widget_wrap">
					<div class="widget_top">
						...
						<h6>Institutes Rights(Adm)(Part-I)</h6>
					</div>
					...
					<div class="widget_content">
						...
						if(isset($_REQUEST['search1']))
						{
							...
							{
								$Rights=explode('.',$row['AdmRights09']);
							?>
							<tr>
...
function assign_value(formId) {
    ...
    form.method = 'post';
    form.action = 'instadm_rights09.php';
    form.submit();
