...
<div id="container">
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action']=='update-record')
	{
		...
		for($i=0; $i < sizeof($BatchesArray); $i++)
		{
			...
			$sql2="UPDATE admbatchstudents10 SET
			AdmStatus			=		".$_REQUEST['NBatchStatus'].",
			RevStatus			=		".$_REQUEST['NBatchStatus']."
			...
			$ins="INSERT INTO tbl_piilog
				...
				SELECT 'AdmRevStatusUpdation-II', 'AdmRevStatus Updated To ".$_REQUEST['NBatchStatus']."', '".$_REQUEST['ActivityRefNo']."', StudentId, ".$_SESSION['emp_id']." 
				FROM admbatchstudents10 WHERE BatchId=".$BatchesArray[$i]."";
			$res=mysql_query($ins, $conn1);
		}//for($i=0; $i < sizeof($BatchesArray); $i++)
		
		?><script>alert('Batch Updated Successfully.');location.replace('admmrevv_radmbatches10.php');</script><?php
	}
	?>
				...
				<div class="widget_wrap">
					...
					<div class="widget_content">
						...
						if(isset($_REQUEST['search']))
						{
							...
							$sql="SELECT Id, Dated, BatchNo, BatchStatus, StdCount, BatchFee, ChallanNo FROM vwadmbatches10 WHERE BatchStatus IN (1, 6) AND BatchType=1 AND AdmStatus=".$_REQUEST['OBatchStatus']." AND RevStatus=".$_REQUEST['OBatchStatus']." AND ExamId=".$ExamId."";
...
function update_batchstatus(form)
{
	...
	else
	{
		if(OBatchStatus != NBatchStatus)
		{
			...
			if(BatchCount > 0)
			{
				location.replace('admmrevv_radmbatches10.php?BatchesArray='+BatchesArray+'&NBatchStatus='+NBatchStatus+'&ActivityRefNo='+ActivityRefNo+'&action=update-record');
			}
			return false;
		}//if(OBatchStatus != NBatchStatus)
		else
