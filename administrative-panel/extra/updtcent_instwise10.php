...
<div id="container">
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action']=='update-record')
	{
		...
		?><script>alert('Exam Centre Updated Successfully.');location.replace('updtcent_instwise10.php');</script><?php
	}
	?>
			...
			<div class="grid_12">
				<div class="widget_wrap">
					...
					<div class="widget_content">
						...
						{
							$SrNo=1; $P=0;
							$sql="SELECT Id, P1RegNo, PRegNo, Name, FatherName, Gender, AdmCategory, SubCategory, CombinationName, GroupName, BatchNo, BatchSr, ACentreName, ACentreCode, ExamShift, InstituteCode, InstituteName FROM vwadmstudents10 WHERE IsRegular=1 AND StdAdmStatus IN (0,1) AND InstituteId=".$_REQUEST['InstituteId']." AND ExamId=".$ExamId."";
...
function update_centre(form)
{
	...
	if(CentreFromId != CentreToId)
	{
		...
		if(Count > 0)
		{
			location.replace('updtcent_instwise10.php?StudentsArray='+StudentsArray+'&CentreToId='+CentreToId+'&ActivityRefNo='+ActivityRefNo+'&action=update-record');
		}
		return false;
	}//if(CentreFromId != CentreToId)
	else
