...
<div id="container">
	...
	<?php
	$sql1="SELECT Id FROM vwadmstudents09 WHERE IsRegular=1 AND SessionId=".$SessionId."";
	$res1=mysql_query($sql1, $conn1);
	$num_rows1=mysql_num_rows($res1);
	
	$sql2="SELECT Id FROM vwadmstudents09 WHERE IsRegular=1 AND SessionId=".$SessionId." AND BatchId is Not NULL";
	$res2=mysql_query($sql2, $conn1);
	$num_rows2=mysql_num_rows($res2);
	
	$sql3="SELECT Id FROM vwadmstudents09 WHERE IsRegular=1 AND SessionId=".$SessionId." AND BatchId is NULL";
					...
					<div class="widget_content">
						...
						<tr>
							...
							<td>
								...
								<option value="All">All</option>
								<?php
								$sql_batch="SELECT Id, BatchNo FROM vwadmbatches09 WHERE BatchType=1 AND ExamId=".$ExamId." ORDER BY BatchNo ASC";
						...
						{
							$SrNo=1;
							$sql="SELECT Id, SSCRegNo, SSCBoard, Dated, Name, FatherName, DOB, Gender, IsSpecial, GroupName, CombinationName, BatchNo, BatchSr, StdAdmStatus, StdRevStatus, AdmissionFee, ACentreCode, RegistrationNo, InstituteCode, InstituteName FROM vwadmstudents09 WHERE IsRegular=1 AND SessionId=".$SessionId."";
							...
							<tr>
								...
								if(in_array('100201',$_SESSION['emp_user_rights'])){?>
									<span><a class="action-icons c-Delete" onClick="return confirm('Are you sure you want to delete this?');" href="deleteallstudents_radm09.php?Id=<?php echo $row['Id'];?>" title="Delete">Delete</a></span>
								<?php }?>
								</td>
							</tr>
							<?php
