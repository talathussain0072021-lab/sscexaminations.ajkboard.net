...
<div id="container">
	...
	if(isset($_REQUEST['submit']))
	{
		...
		{
			$sql2="DELETE FROM vwrollnoslip09 WHERE Id=".$_REQUEST['Id']."";
			$res2=mysql_query($sql2, $conn1);
			
			$ins="INSERT INTO tbl_pilog SET
			ActivityType			=		'RollNoSlipDeletion-I',
			ActivityDescription		=		'Record of RollNoSlip - SSC-I is deleted from DB',
			...
			if($_REQUEST['Type'] == 1)
			{ ?><script>location.replace('regstudents-rollnoslips09.php?message=Data Updated Successfully.');</script><?php }
			else if($_REQUEST['Type'] == 0)
			{ ?><script>location.replace('privstudents-rollnoslips09.php?message=Data Updated Successfully.');</script><?php }
		}
		else
		{
			?><script>alert('Error in Query.');location.replace('delete_rollnoslips09.php?Id=<?php echo $_REQUEST['Id'];?>&Type=<?php echo $_REQUEST['Type'];?>');</script><?php
		}
	}
	?>
	<?php
	$sql="SELECT Id, Name, FatherName FROM vwrollnoslip09 WHERE Id=".$_REQUEST['Id']."";
