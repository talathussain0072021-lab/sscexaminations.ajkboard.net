...
<div id="container">
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action']=='update-record')
	{
		...
		?><script>alert('Exam Shift Updated Successfully.');location.replace('updtshift_centwise09s.php');</script><?php
	}
	?>
			...
			<div class="grid_12">
				<div class="widget_wrap">
					<div class="widget_top">
						...
						<h6>Change Shift(Centre Wise)</h6>
					</div>
					...
					<div class="widget_content">
						...
						{
							$SrNo=1; $P=0;
							$sql="SELECT Id, P1RegNo, Name, FatherName, Gender, IsRegular, AdmCategory, SubCategory, CombinationName, GroupName, BatchNo, BatchSr, ACentreName, ACentreCode, ExamShift, InstituteCode, InstituteName FROM vwadmstudents09s WHERE StdAdmStatus IN (0,1) AND ACentreId=".$_REQUEST['CentreFromId']." AND ExamShift=".$_REQUEST['ShiftFrom']." AND ExamId=".$ExamId."";
...
function update_shift(form)
{
	...
	if(ShiftFrom != ShiftTo)
	{
		...
		if(Count > 0)
		{
			location.replace('updtshift_centwise09s.php?StudentsArray='+StudentsArray+'&ShiftTo='+ShiftTo+'&ActivityRefNo='+ActivityRefNo+'&action=update-record');
		}
		return false;
	}//if(ShiftFrom != ShiftTo)
	else
