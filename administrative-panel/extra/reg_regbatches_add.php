...
<div id="container">
	...
	if(isset($_REQUEST['submit1']))
	{
		...
		if($num_rows > 0)
		{
			...
			?><script>alert('Batch Updated Successfully.');location.replace('reg_regbatches_add.php');</script><?php
		}
		else
		{ ?><script>alert('Batch not Found.');location.replace('reg_regbatches_add.php');</script><?php }
	}//if(isset($_REQUEST['submit1']))
					...
					<div class="widget_content">
						<form action="" method="post" class="form_container left_label" onSubmit="return check_submit_form();" enctype="multipart/form-data">
							<ul>
								...
								<li>
								<div class="form_grid_12">
									<label class="field_title">Reg Status</label>
									<div class="form_input">
										<input name="RegStatus" id="RegStatus" type="text" class="limiter" readonly/>
									</div>
								</div>
								</li>
...
function update_record()
{
	...
	if(BatchInfo.BatchStatus == 1 || BatchInfo.BatchStatus == 6)
	{
		...
		if(BatchInfo.RegStatus == 1){ $("#RegStatus").val("OK"); }
		else if(BatchInfo.RegStatus == 2){ $("#RegStatus").val("NOT OK"); }
		else if(BatchInfo.RegStatus == 0){ $("#RegStatus").val("PENDING"); }
	}
	else
	{
		$("#BatchNo").val("");
