...
Sub5Name, Sub5Day, Sub5Date, Sub5Time, Sub6Name, Sub6Day, Sub6Date, Sub6Time, Sub7Name, Sub7Day, Sub7Date, Sub7Time,
Sub8Name, Sub8Day, Sub8Date, Sub8Time, Sub9Name, Sub9Day, Sub9Date, Sub9Time
FROM vwrollnoslip09 WHERE 1=1";

if(isset($_REQUEST['Id']) && $_REQUEST['Id']!='') { 
	$sql.=" AND Id=".$_REQUEST['Id']."";
}
else {
	if(isset($_REQUEST['IsRegular']) && $_REQUEST['IsRegular']!='')
...
$DateArray = array($row['Sub1Date'], $row['Sub2Date'], $row['Sub3Date'], $row['Sub5Date'], $row['Sub6Date'], $row['Sub7Date'], $row['Sub8Date'], $row['Sub9Date']);
$SubNameArray = array($row['Sub1Name'], $row['Sub2Name'], $row['Sub3Name'], $row['Sub5Name'], $row['Sub6Name'], $row['Sub7Name'], $row['Sub8Name'], $row['Sub9Name']);
$SubDayArray = array($row['Sub1Day'], $row['Sub2Day'], $row['Sub3Day'], $row['Sub5Day'], $row['Sub6Day'], $row['Sub7Day'], $row['Sub8Day'], $row['Sub9Day']);
$SubTimeArray = array($row['Sub1Time'], $row['Sub2Time'], $row['Sub3Time'], $row['Sub5Time'], $row['Sub6Time'], $row['Sub7Time'], $row['Sub8Time'], $row['Sub9Time']);
...
$RestArraySize = max(0, 4 - $DateArraySize);
$lineCount = $DateArraySize + $RestArraySize;

// Adjust these values based on actual heights
$rowHeight = 17; // Measured row height including borders/padding
$barcodeHeight = 30; // Approximate barcode height
$imageHeight = 50; // As defined in the image's style

// Calculate total height of the left table (including header row)
$totalLeftRows = 1 + $lineCount; // 1 header row + data rows
$totalLeftHeight = $totalLeftRows * $rowHeight;

// Margin for the image to align with the bottom of the left table
$marginTop = max(0, $totalLeftHeight - $barcodeHeight - $imageHeight);
...
$html.='<div style="width:79%; float:left;">
	...
	$html.='<tr>
				...
				<td>'.strtoupper($SubDayArray[$i]).'</td>
...
$html.='</table>
		...
		<div style="width:20%; float:right;">
			...
			<div align="center"><img src="images/controller.jpg" style="height:50px; width:100px; margin-top: '.$marginTop.'px;"/></div>
		</div>';
...
$html.='<div style="width:100%; float:left;">
		<table width="100%" border="0" align="center" style="font-family:sans-serif; text-align:left; border-collapse:collapse; font-size:8.5pt; margin-top:1px;">
			...
			<tr>
				<td colspan="4" align="center"><img src="images/pi_rollnoslip.jpg"/></td>
			</tr>
		</table>
		</div><hr/>';
