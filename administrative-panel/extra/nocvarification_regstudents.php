...
<div id="container">
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action']=='update-record')
	{
		...
		NOCVerified			=		".$_REQUEST['NNOCVerStatus']."
		...
		$ins="INSERT INTO tbl_pilog (ActivityType, ActivityDescription, ActivityRefNo, StudentId, EmployeeId)
			SELECT 'NOCVerStatusUpdation-I', 'VerStatus Updated To ".$_REQUEST['NNOCVerStatus']."', '".$_REQUEST['ActivityRefNo']."', StudentId, ".$_SESSION['emp_id']." FROM regbatchstudents WHERE StudentId IN (".$_REQUEST['StudentsArray'].")";
				...
				<div class="widget_wrap">
					...
					<div class="widget_content">
						...
						<tr>
							...
							<td><strong>NOC VerStatus:</strong></td>
							<td>
								<select name="ONOCVerStatus" id="ONOCVerStatus" data-placeholder="Select NOCVerStatus" class="chzn-select admin-select" tabindex="2"/>
								<option value="0" <?php echo (($_REQUEST['ONOCVerStatus']==0)?'selected':'')?>>Pending</option>
								<option value="1" <?php echo (($_REQUEST['ONOCVerStatus']==1)?'selected':'')?>>Ok</option>
								<option value="2" <?php echo (($_REQUEST['ONOCVerStatus']==2)?'selected':'')?>>Not Ok</option>
								</select>
							</td>
						...
						<tr>
							<td><strong>NOC VerStatus:</strong></td>
							<td>
								<select name="NNOCVerStatus" id="NNOCVerStatus" data-placeholder="Select NOCVerStatus" class="chzn-select admin-select" tabindex="4"/>
						...
						{
							$SrNo=1; $P=0;
							$sql="SELECT Id, Name, FatherName, Gender, AdmissionType, GroupName, BatchNo, RegistrationFee, BatchSr, RegInstituteCode, RegInstituteName FROM vwregstudents WHERE AdmissionType=4 AND NOCVerified=".$_REQUEST['ONOCVerStatus']." AND SessionId=".$SessionId."";
...
function update_studentstatus(form)
{
	...
	var NNOCVerStatus=document.getElementById('NNOCVerStatus').value;
	...
	else
	{
		...
		if(StdCount > 0)
		{
			location.replace('nocvarification_regstudents.php?StudentsArray='+StudentsArray+'&NNOCVerStatus='+NNOCVerStatus+'&ActivityRefNo='+ActivityRefNo+'&action=update-record');
		}
