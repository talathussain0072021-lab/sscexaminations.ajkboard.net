...
<div id="container">
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action']=='update-record')
	{
		...
		for($i=0; $i < sizeof($InstitutesArray); $i++)
		{
			...
			RegRights		=		'".$RightsArray[$i]."'
			WHERE Id		=		".$InstitutesArray[$i]."";
			$res_q=mysql_query($sql_q, $conn1);
		}
		
		?><script>alert('Rights Updated Successfully.');location.replace('instreg_rights.php');</script><?php
	}
	?>
				...
				<div class="widget_wrap">
					...
					<div class="widget_content">
						...
						<tr>
							...
							<th>Code</th>
							<th>Name</th>
							<th>Add Student <input type="checkbox" name="checkbox1" id="checkboxa1" value="" onclick="CheckAll(this)"/></th>
							<th>Edit Student <input type="checkbox" name="checkbox2" id="checkboxa2" value="" onclick="CheckAll(this)"/></th>
							<th>Delete Student <input type="checkbox" name="checkbox3" id="checkboxa3" value="" onclick="CheckAll(this)"/></th>
							<th>Create Batch <input type="checkbox" name="checkbox4" id="checkboxa4" value="" onclick="CheckAll(this)"/></th>
							<th>Show Batch <input type="checkbox" name="checkbox5" id="checkboxa5" value="" onclick="CheckAll(this)"/></th>
						</tr>
						</thead>
						<tbody>
						<?php
						if(isset($_REQUEST['search1']))
						{
							...
							<tr>
								...
								<td class="center">
								<input type="checkbox" name="checkbox4" id="checkbox4_<?php echo $row['Id'];?>" value="40" <?php echo (in_array('40',$Rights))?'checked="checked"':'';?>>
								</td>
								<td class="center">
								<input type="checkbox" name="checkbox5" id="checkbox5_<?php echo $row['Id'];?>" value="50" <?php echo (in_array('50',$Rights))?'checked="checked"':'';?>>
								</td>
							</tr>
							<?php
							$SrNo++;
...
function assign_value(formId) {
    ...
    form.method = 'post';
    form.action = 'instreg_rights.php';
    form.submit();
