...
<div id="container">
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action']=='update-record')
	{
		...
		*/
		?><script>alert('Pictures Status Updated Successfully.');location.replace('allstudents_instpanel.php');</script><?php
	}
	?>
				...
				<div class="widget_wrap">
					...
					<div class="widget_content">
						...
						<tr>
							...
							<td>
								<select name="SSCBoard" id="SSCBoard" data-placeholder="Select SSC Board" class="chzn-select admin-select" tabindex="2"/>
								<option value="All">All</option>
							...
							<td>
								<select name="AdmissionType" id="AdmissionType" data-placeholder="Select Admission Type" class="chzn-select admin-select" tabindex="3"/>
								<option value="All">All</option>
						...
						<tr>
							...
							<td>
								<select name="pic_value" id="pic_value" data-placeholder="Select" class="chzn-select" style="width:135px;" tabindex="7"/>
						...
						{
							$SrNo=1; $P=0;
							$sql="SELECT Id, SSCRegNo, SSCBoard, Dated, Name, FatherName, DOB, PicURL, Gender, IsSpecial, IsRegular, AdmissionType, GroupName, InstituteId, InstituteCode, InstituteName FROM vwregstudents WHERE InstituteId!=0 AND SessionId=".$SessionId."";
...
function assign_value(form)
{
	...
	if(count > 0)
	{
		location.replace('allstudents_instpanel.php?students_array='+students_array+'&pic_value='+pic_value+'&action=update-record');
	}
	return false;
}
</script>