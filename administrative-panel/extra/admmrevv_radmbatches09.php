...
<div id="container">
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action']=='update-record')
	{
		...
		for($i=0; $i < sizeof($BatchesArray); $i++)
		{
			...
			$sql2="UPDATE admbatchstudents09 SET
			AdmStatus			=		".$_REQUEST['NBatchStatus'].",
			RevStatus			=		".$_REQUEST['NBatchStatus']."
			...
			$ins="INSERT INTO tbl_pilog 
				...
				FROM admbatchstudents09 WHERE BatchId=".$BatchesArray[$i]."";
			$res=mysql_query($ins, $conn1);
		}//for($i=0; $i < sizeof($BatchesArray); $i++)
		
		?><script>alert('Batch Updated Successfully.');location.replace('admmrevv_radmbatches09.php');</script><?php
	}
	?>
				...
				<div class="widget_wrap">
					...
					<div class="widget_content">
						...
						if(isset($_REQUEST['search']))
						{
							...
							$sql="SELECT Id, Dated, BatchNo, BatchStatus, StdCount, BatchFee, ChallanNo FROM vwadmbatches09 WHERE BatchStatus IN (1, 6) AND BatchType=1 AND AdmStatus=".$_REQUEST['OBatchStatus']." AND RevStatus=".$_REQUEST['OBatchStatus']." AND ExamId=".$ExamId."";
...
function update_batchstatus(form)
{
	...
	else
	{
		if(OBatchStatus != NBatchStatus)
		{
			...
			if(BatchCount > 0)
			{
				location.replace('admmrevv_radmbatches09.php?BatchesArray='+BatchesArray+'&NBatchStatus='+NBatchStatus+'&ActivityRefNo='+ActivityRefNo+'&action=update-record');
			}
			return false;
		}//if(OBatchStatus != NBatchStatus)
		else
