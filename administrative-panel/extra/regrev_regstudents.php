...
<div id="container">
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action']=='update-record')
	{
		...
		if($_REQUEST['NRegRevStatus'] == 1)
		{
			...
			IsRegistered		=		1
			WHERE Id			IN		(".$_REQUEST['StudentsArray'].")";
		...
		else
		{
			...
			WHERE Id			IN		(".$_REQUEST['StudentsArray'].")";
			$res2=mysql_query($sql2, $conn1);
		}//elseif($_REQUEST['NRegRevStatus'] == 1)
		
		$ins="INSERT INTO tbl_pilog 
						(ActivityType, ActivityDescription, ActivityRefNo, StudentId, EmployeeId)
			SELECT 'RegRevStatusUpdation-I', 'RegRevStatus Updated To ".$_REQUEST['NRegRevStatus']."', '".$_REQUEST['ActivityRefNo']."', StudentId, ".$_SESSION['emp_id']." 
			FROM regbatchstudents WHERE StudentId IN (".$_REQUEST['StudentsArray'].")";
		$res=mysql_query($ins, $conn1);
		
		?><script>alert('Students Updated Successfully.');location.replace('regrev_regstudents.php');</script><?php
	}
	?>
					...
					<div class="widget_content">
						...
						<tr>
							...
							<td>
								...
								<?php
								$sql_batch="SELECT Id, BatchNo FROM vwregbatches WHERE BatchStatus IN (1, 6) AND SessionId=".$SessionId." ORDER BY BatchNo ASC";
							...
							<td>
								<select name="ORegRevStatus" id="ORegRevStatus" data-placeholder="Select Student Status" class="chzn-select admin-select" tabindex="3"/>
								<option value="0" <?php echo (($_REQUEST['ORegRevStatus']==0)?'selected':'')?>>Pending</option>
								<option value="1" <?php echo (($_REQUEST['ORegRevStatus']==1)?'selected':'')?>>Ok</option>
								<option value="2" <?php echo (($_REQUEST['ORegRevStatus']==2)?'selected':'')?>>Not Ok</option>
								</select>
							</td>
						...
						<tr>
							<td><strong>Reg/Rev Status:</strong></td>
							<td>
								<select name="NRegRevStatus" id="NRegRevStatus" data-placeholder="Select Student Status" class="chzn-select admin-select" tabindex="5"/>
								<option value="0" <?php echo (($_REQUEST['NRegRevStatus']==0)?'selected':'')?>>Pending</option>
								<option value="1" <?php echo (($_REQUEST['NRegRevStatus']==1)?'selected':'')?>>Ok</option>
								<option value="2" <?php echo (($_REQUEST['NRegRevStatus']==2)?'selected':'')?>>Not Ok</option>
								</select>
							</td>
						...
						{
							$SrNo=1; $P=0;
							$sql="SELECT Id, Name, FatherName, Gender, Domicile, GroupName, CombinationName, BatchNo, BatchSr, RegistrationFee, InstituteCode, InstituteName FROM vwregstudents WHERE StdRegStatus=".$_REQUEST['ORegRevStatus']." AND StdRevStatus=".$_REQUEST['ORegRevStatus']." AND SessionId=".$SessionId."";
...
function update_studentstatus(form)
{
	...
	var ORegRevStatus=document.getElementById('ORegRevStatus').value;
	var NRegRevStatus=document.getElementById('NRegRevStatus').value;
	...
	{
		if(ORegRevStatus != NRegRevStatus)
		{
			for(var i = 0; i < form.elements.length; i++)
			{
				if(form.elements[i].type === "checkbox" && form.elements[i].checked == true && form.elements[i].value != '')
			...
			if(StdCount > 0)
			{
				location.replace('regrev_regstudents.php?StudentsArray='+StudentsArray+'&NRegRevStatus='+NRegRevStatus+'&ActivityRefNo='+ActivityRefNo+'&action=update-record');
			}
			return false;
		}//if(ORegRevStatus != NRegRevStatus)
		else
