...
<div id="container">
	...
	if(isset($_REQUEST['submit']))
	{
		...
		InternalGrade			=		'".$_REQUEST['InternalGrade']."'
		WHERE Id				=		".$_REQUEST['Id']."";
		
		if(mysql_query($sql, $conn1))
		{
			$ins="INSERT INTO tbl_piilog SET
			ActivityType			=		'PersonalInfoUpdation-II',
		...
		{
			?><script>alert('Error in Query.');location.replace('updtpersonalinfo_student10.php?Id=<?php echo $_REQUEST['Id'];?>');</script><?php
		}
	}
	?>
	<?php
	$sql="SELECT Id, Name, FatherName, DOB, CNIC, Gender, Religion, IdentityMarks, IsSpecial, InternalGrade, AdmCategory, SubCategory FROM vwadmstudents10 WHERE Id=".$_REQUEST['Id']."";
						...
						<form action="" method="post" class="form_container left_label" onSubmit="return check_submit_form();" enctype="multipart/form-data">
							<ul>
								...
								<fieldset>
									...
									<ul>
										<?php if($AdmSubCategory == '11' || $AdmSubCategory == '31' || $AdmSubCategory == '41' || $AdmSubCategory == '51' || $AdmSubCategory == '61' || $AdmSubCategory == '75' || $AdmSubCategory == '95'){?>
										<li>
										...
										<li>
										<div class="form_grid_6">
											<label class="field_title">Internal Grade</label>
											<div class="form_input">
												<select name="InternalGrade" id="InternalGrade" data-placeholder="Select Internal Grade" class="chzn-select custom-select" tabindex="9">
												<option value="">Select</option>
												<option value="A" <?php echo (($row['InternalGrade']=='A')?'selected':'');?>>A</option>
												<option value="B" <?php echo (($row['InternalGrade']=='B')?'selected':'');?>>B</option>
												<option value="C" <?php echo (($row['InternalGrade']=='C')?'selected':'');?>>C</option>
												<option value="D" <?php echo (($row['InternalGrade']=='D')?'selected':'');?>>D</option>
												<option value="E" <?php echo (($row['InternalGrade']=='E')?'selected':'');?>>E</option>
												</select>
											</div>
										</div>
										<div class="form_grid_6">
											...
											<div class="form_input">
												<input name="ActivityRefNo" id="ActivityRefNo" type="text" data-required="required" data-message="Enter Letter No" class="x_large" maxlength="50" tabindex="10"/>
											</div>
										</div>
										<br /><br /><br />
										</li>
										...
										<div class="form_grid_12">
											<div class="form_input">
												...
												<button type="submit" name="submit" value="submit" class="btn_small btn_blue" tabindex="11"><span>Update</span></button>
												<button type="button" name="reset" class="btn_small btn_blue" onclick="location.replace('updtpersonalinfo_student10.php?Id=<?php echo $_REQUEST['Id'];?>')" tabindex="12"><span>Reset</span></button>
											</div>
										</div>
										</li>
									</ul>
