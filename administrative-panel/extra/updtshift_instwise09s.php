...
<div id="container">
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action']=='update-record')
	{
		...
		?><script>alert('Exam Shift Updated Successfully.');location.replace('updtshift_instwise09s.php');</script><?php
	}
	?>
...
function update_shift(form)
{
	...
	if(ShiftFrom != ShiftTo)
	{
		...
		if(Count > 0)
		{
			location.replace('updtshift_instwise09s.php?StudentsArray='+StudentsArray+'&ShiftTo='+ShiftTo+'&ActivityRefNo='+ActivityRefNo+'&action=update-record');
		}
		return false;
	}//if(ShiftFrom != ShiftTo)
	else
