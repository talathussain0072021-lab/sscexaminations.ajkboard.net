...
<div id="container">
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action']=='update-record')
	{
		...
		for($i=0; $i < sizeof($StudentsArray); $i++)
		{
			...
			ActivityType			=		'AdmRevStatusUpdation-II',
		...
		?><script>alert('Students Updated Successfully.');location.replace('admrev_admstudents10.php');</script><?php
	}
	?>
					...
					<div class="widget_content">
						...
						<tr>
							...
							<td>
								...
								<?php
								$sql_batch="SELECT Id, BatchNo FROM vwadmbatches10 WHERE BatchStatus IN (1, 6) AND ExamId=".$ExamId." ORDER BY BatchNo ASC";
						...
						{
							$SrNo=1; $P=0;
							$sql="SELECT Id, Name, FatherName, Gender, Domicile, GroupName, CombinationName, BatchNo, BatchSr, AdmissionFee, ACentreCode, InstituteCode, InstituteName FROM vwadmstudents10 WHERE StdAdmStatus=".$_REQUEST['OAdmRevStatus']." AND StdRevStatus=".$_REQUEST['OAdmRevStatus']." AND ExamId=".$ExamId."";
...
function update_studentstatus(form)
{
	...
	else
	{
		if(OAdmRevStatus != NAdmRevStatus)
		{
			...
			if(StdCount > 0)
			{
				location.replace('admrev_admstudents10.php?StudentsArray='+StudentsArray+'&NAdmRevStatus='+NAdmRevStatus+'&ActivityRefNo='+ActivityRefNo+'&action=update-record');
			}
			return false;
		}//if(OAdmRevStatus != NAdmRevStatus)
		else
