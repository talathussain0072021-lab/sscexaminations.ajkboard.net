...
<div id="container">
	...
	if(isset($_REQUEST['update']))
	{
		...
		if ($rowVer['cnt'] != 2) {
			echo "<script>alert('No records found for New ChallanNo with paid status OR Old ChallanNo with unpaid status.');location.replace('updtchlnno_admstudents09s.php');</script>"; exit;
		}

		// Step 1: Get affected rows BEFORE updating
		$affectedStudents = array();

		$sqlGet = "SELECT StudentId FROM admbatchstudents09s
		...
		if (empty($affectedStudents)) {
			echo "<script>location.replace('updtchlnno_admstudents09s.php?message=No records found for the given Old ChallanNo.');</script>"; exit;
		}

		// Step 2: UPDATE in admbatchstudents09s
		$sql1 = "UPDATE admbatchstudents09s SET
				 ChallanNo = ".$_REQUEST['NChallanNo']."
				 WHERE ChallanNo = ".$_REQUEST['OChallanNo'];

		if (mysql_query($sql1, $conn1)) {

			// Step 3: UPDATE in admbatches09s
			$sql2 = "UPDATE admbatches09s SET
					 ChallanNo = ".$_REQUEST['NChallanNo']."
					 WHERE ChallanNo = ".$_REQUEST['OChallanNo'];
			...
			foreach ($affectedStudents as $sid) {
				$ins = "INSERT INTO tbl_pilog 
						...
						VALUES (
							'ChallanNoUpdation-Is',
							'AdmChallanNo updated from ".$_REQUEST['OChallanNo']." to ".$_REQUEST['NChallanNo']."',
			...
			?><script>location.replace('updtchlnno_admstudents09s.php?message=Data Updated Successfully.');</script><?php
		}
		else
		{
			?><script>alert('Error in Query.');location.replace('updtchlnno_admstudents09s.php');</script><?php
		}
	}
	?>
				...
				<div class="widget_wrap">
					...
					<div class="widget_content">
						...
						if(isset($_REQUEST['search']))
						{
							...
							$sql="SELECT Id, Name, FatherName, BatchNo, BatchSr, StdAdmStatus, AdmissionFee FROM vwadmstudents09s WHERE ChallanNo=".$_REQUEST['OChallanNo']." ORDER BY Id ASC";
							...
							<tr>
								...
								<td class="center"><?php echo $row['AdmissionFee'];?></td>
							</tr>
							<?php
							$SrNo++; $StdCountSum=$numrows; $FeeSum+=$row['AdmissionFee'];
							}//while($row=mysql_fetch_array($res))
						}?>
						</tbody>
						<tfoot>
