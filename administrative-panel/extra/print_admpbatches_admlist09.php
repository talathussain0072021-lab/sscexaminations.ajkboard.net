...
<tr>
	...
	<td style="font-size:14pt; text-align:center; width:80%; vertical-align:middle;"><b>ADM. BATCHES(PRIV) REPORT '.$SessionName.'</b></td>
...
<tr>
	...
	<td style="background-color:#D1E2F2; width:5%;"><strong>Batch No.</strong></td>
	<td style="background-color:#D1E2F2; width:5%;"><strong>Student Count</strong></td>
	<td style="background-color:#D1E2F2; width:30%;"><strong>Sr.No.</strong></td>	
	<td style="background-color:#D1E2F2; width:55%;"><strong>App. No.</strong></td>           	
</tr></table>','O');
		...
		$SrNo=$first_number+1; $BatchSum=0; $StudentSum=0;
		$sql="SELECT Id, BatchNo, StdCount FROM vwadmbatches11 WHERE BatchStatus=1 AND BatchType=2 AND SessionId=".$SessionId." AND AdmStatus=".$_REQUEST['BatchStatus']." ORDER BY BatchNo ASC limit {$first_number}, 500";
		$res=mysql_query($sql, $conn1);
		while($row=mysql_fetch_array($res))
		{			
			$AppArray=''; $BatchSrArray=''; //$centres_array='';
			$sql2="SELECT Id FROM vwadmstudents11 WHERE BatchId=".$row['Id']."";
			$res2=mysql_query($sql2, $conn1);
			while($row2=mysql_fetch_array($res2))
			{ $AppArray.=$row2['Id'].', '; }
			
			$sql3="SELECT BatchSr FROM vwadmstudents11 WHERE BatchId=".$row['Id']."";
			$res3=mysql_query($sql3, $conn1);
			while($row3=mysql_fetch_array($res3))
			{ $BatchSrArray.=$row3['BatchSr'].', '; }
											
			$html.='<tr>
						<td style="width:5%;">'.$SrNo.'</td>
						<td style="width:5%;">'.$row['BatchNo'].'</td>
						<td style="width:5%;">'.$row['StdCount'].'</td>
						<td style="width:30%;">'.$BatchSrArray.'</td>
						<td style="width:55%;">'.$AppArray.'</td>							
					</tr>';
					$SrNo++; $BatchSum++;
					$StudentSum+=$row['StdCount'];
		}
			$html.='<tr>
						<td colspan="3" style="font-weight:bold;"> Total Batches: '.$BatchSum.'</td>
						<td colspan="2" style="font-weight:bold;"> Total Students: '.$StudentSum.'</td>						
					</tr>';
