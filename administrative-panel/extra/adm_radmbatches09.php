...
<div id="container">
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action']=='update-record')
	{
		...
		?><script>alert('Batch Updated Successfully.');location.replace('adm_radmbatches09.php');</script><?php
		}//if($_REQUEST['BatchStatus'] == 1)
					...
					<div class="widget_content">
						...
						<div class="invoice_action_bar" style="float: right;">
                        	<div class="btn_30_blue">
                        		<a href="adm_radmbatches_add09.php"><span class="icon add_co"></span>
						...
						if(isset($_REQUEST['search']))
						{
							...
							$sql="SELECT Id, BatchNo, BatchStatus, AdmStatus, RevStatus, BatchCounter, InstituteId, StdCount, ChallanNo FROM vwadmbatches09 WHERE BatchStatus=1 AND BatchType=1 AND AdmStatus=0 AND ExamId=".$ExamId." AND InstituteId=".$_REQUEST['InstituteId']." ORDER BY Id ASC";
						...
						{
							$SrNo=1; $BatchSum=0;
							$sql="SELECT Id, BatchNo, BatchStatus, RegStatus, RevStatus, BatchCounter, InstituteId, StdCount, ChallanNo FROM vwadmbatches09 WHERE BatchStatus=1 AND BatchType=1 AND AdmStatus=0 AND SessionId=".$SessionId." ORDER BY Id ASC";
							$res=mysql_query($sql, $conn1);
							while($row=mysql_fetch_array($res))
							{?>
							<tr>
...
function update_batchstatus(form)
{
	...
	else
	{
		...
		if(BatchCount > 0)
		{
			location.replace('adm_radmbatches09.php?BatchesArray='+BatchesArray+'&BatchStatus='+BatchStatus+'&action=update-record');
		}
