...
<div id="container">
	...
	{
		$sql="DELETE FROM students10 WHERE Id=".$_REQUEST['Id']."";
		...
		if(mysql_query($sql, $conn1))
		{
			...
			ActivityType			=		'Deletion-II',
			ActivityDescription		=		'Record of ".$_REQUEST['Name'].' - '.$_REQUEST['GroupId'].' - SSC-II is deleted from DB'."',
			ActivityRefNo			=		'".$_REQUEST['ActivityRefNo']."',
			StudentId				=		".$_REQUEST['Id'].",
			EmployeeId				=		".$_SESSION['emp_id']."";
			$res=mysql_query($ins, $conn1);
			
			?><script>alert('Student Deleted Successfully.');location.replace('allstudents_delete10.php');</script><?php
		}
		else
		{
			?><script>alert('Error in Query.');location.replace('delete_student10.php?Id=<?php echo $_REQUEST['Id'];?>');</script><?php
		}
	}
	?>
	<?php
	$sql="SELECT Id, P1StudentId, Name, FatherName, GroupId FROM vwadmstudents10 WHERE Id=".$_REQUEST['Id']."";
										...
										<ul>
											...
											<div class="form_grid_12">
												<div class="form_input">
													...
													<button type="button" name="reset" class="btn_small btn_blue" onclick="location.replace('delete_student10.php?Id=<?php echo $_REQUEST['Id'];?>')" tabindex="3"><span>Reset</span></button>
												</div>
											</div>
											</li>
										</ul>
