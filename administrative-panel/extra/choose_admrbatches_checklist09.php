...
<?php
$sql="SELECT count(Id) as tbatch_count FROM vwadmbatches11 WHERE BatchStatus=".$_REQUEST['BatchStatus']." AND BatchType=1 AND SessionId=".$_REQUEST['SessionId']."";
if($_REQUEST['id'] == 1)
					...
					<div class="widget_content">
							<table height="200px" width="70%" style="margin-left:20px;" class="tablarge_property">
								...
								{ 
									...
									?>
									<tr height="50px">
									<td align="left" colspan="4" style="font-weight:bolder; font-family:Verdana, Arial, Helvetica, sans-serif; font-size:12px;">-> <a href="#" onclick="parent.jQuery.colorbox.close();window.open('<?php if($_REQUEST['id']==1){?>print_admrbatches_admchecklist11.php<?php } else if($_REQUEST['id']==2){?>print_admrbatches_revchecklist11.php<?php } else if($_REQUEST['id']==3){?>print_admrbatches_finchecklist11.php<?php }?>?&BatchStatus=<?php echo $_REQUEST['BatchStatus'];?>&AdmRevStatus=<?php echo $_REQUEST['AdmRevStatus'];?>&first_number=<?php echo $first_number;?>&&last_number=<?php echo $last_number;?>','_parent');">PRINT RECORD FROM <?php echo $first_number.'-'.$last_number;?></a></td>
									</tr>
								<?php	
								}
								?>
