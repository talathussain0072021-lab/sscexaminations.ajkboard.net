...
<div id="container">
	...
	if(isset($_REQUEST['update']))
	{
		...
		if ($rowVer['cnt'] != 2) {
			echo "<script>alert('No records found for New ChallanNo with paid status OR Old ChallanNo with unpaid status.');location.replace('updtchlnno_admstudents09.php');</script>"; exit;
		}

		// Step 1: Get affected rows BEFORE updating
		$affectedStudents = array();

		$sqlGet = "SELECT StudentId FROM admbatchstudents09 
		...
		if (empty($affectedStudents)) {
			echo "<script>location.replace('updtchlnno_admstudents09.php?message=No records found for the given Old ChallanNo.');</script>"; exit;
		}

		// Step 2: UPDATE in admbatchstudents09
		$sql1 = "UPDATE admbatchstudents09 SET
		...
		if (mysql_query($sql1, $conn1)) {
			...
			?><script>location.replace('updtchlnno_admstudents09.php?message=Data Updated Successfully.');</script><?php
		}
		else
		{
			?><script>alert('Error in Query.');location.replace('updtchlnno_admstudents09.php');</script><?php
		}
	}
	?>
				...
				<div class="widget_wrap">
					...
					<div class="widget_content">
						...
						if(isset($_REQUEST['search']))
						{
							...
							$sql="SELECT Id, Name, FatherName, BatchNo, BatchSr, StdAdmStatus, AdmissionFee FROM vwadmstudents09 WHERE ChallanNo=".$_REQUEST['OChallanNo']." ORDER BY Id ASC";
