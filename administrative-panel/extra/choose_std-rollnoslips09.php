...
<?php
$sql="SELECT count(Id) as tstd_count FROM vwrollnoslip09 WHERE 1=1";
				...
				<!--<a href="voucher_print.php?id=<?php //echo $_REQUEST['id'];?>">
					...
					<div class="widget_content">
						...
						while($last_number < $max_number)
						{
							...
							if(($first_number+99) > $max_number)
							{ $last_number= $last_number+($max_number-$last_number); }
							else
							//{ $last_number=$first_number+149; }
							{ $last_number=$first_number+99; }
						?>
						<tr height="40px">
							<td>-> <a href="print_student_allrollnoslips09.php?StdudentId=<?php echo $_REQUEST['StdudentId'];?>&&RegistrationNo=<?php echo $_REQUEST['RegistrationNo'];?>&&RollNo=<?php echo $_REQUEST['RollNo'];?>&&BatchSr=<?php echo $_REQUEST['BatchSr'];?>&&Name=<?php echo $_REQUEST['Name'];?>&&CentreCode=<?php echo $_REQUEST['CentreCode'];?>&&InstituteCode=<?php echo $_REQUEST['InstituteCode'];?>&&IsRegular=<?php echo $_REQUEST['IsRegular'];?>&&first_number=<?php echo $first_number;?>&&last_number=<?php echo $last_number;?>" target="_blank">PRINT ROLLNO SLIP FROM <?php echo $first_number.'-'.$last_number;?></a></td>
						</tr>
						<?php
						}
						?>
