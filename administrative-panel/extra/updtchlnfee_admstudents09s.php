...
<div id="container">
	...
	if (isset($_GET['action']) && $_GET['action'] == 'update-fee') {
		...
		if ($rowVer['cnt'] >= 1) {
			echo "<script>alert('ChallanFee is already paid.');location.replace('updtchlnfee_admstudents09s.php');</script>"; exit;
		}
		...
		for ($i = 0; $i < count($ids); $i++) {
			...
			ActivityType			=		'ChallanFeeUpdation-Is',
			ActivityRefNo			=		'".$_GET['ActivityRefNo']."',
			ActivityDescription		=		'AdmChallanFee Updated FROM ".$oldfees[$i]." To ".$newfees[$i]."',
			StudentId				=		".$ids[$i].",
		...
		?><script>location.replace('updtchlnfee_admstudents09s.php?message=Data Updated Successfully.');</script><?php
	}
	?>
				...
				<div class="widget_wrap">
					...
					<div class="widget_content">
						...
						if(isset($_REQUEST['search']))
						{
							...
							$sql="SELECT Id, Name, FatherName, BatchNo, BatchSr, StdAdmStatus, AdmissionFee FROM vwadmstudents09s WHERE ChallanNo=".$_REQUEST['ChallanNo']." ORDER BY Id ASC";
...
function validateUpdate(form)
{
    ...
    if (IdList.length === 0) {
		...
		location.replace(
			'updtchlnfee_admstudents09s.php?ids=' + idsJSON + '&oldfees=' + oldfeesJSON + '&newfees=' + newfeesJSON + '&ChallanNo='+ ChallanNo +
