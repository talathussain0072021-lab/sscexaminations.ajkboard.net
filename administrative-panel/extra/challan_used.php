<?php include('includes/top.php');?>
	...
	{
		$sql="SELECT Id FROM challan_used WHERE ChallanNo=".$_REQUEST['ChallanNo']."";
		$res=mysql_query($sql, $conn2);
		$num_rows=mysql_num_rows($res);
		if($num_rows > 0){
			
			$insc="UPDATE challan_used SET
			...
			if($resc){
				$insc1="INSERT INTO log_challanused SET
		...
		{
			$insc="INSERT INTO challan_used SET
			ChallanNo			=	".$_REQUEST['ChallanNo'].",
			IsUsed				=	".$_REQUEST['IsUsed'].",
			DepartmentId		=	".$_REQUEST['DepartmentId']."";
			$resc=mysql_query($insc, $conn2);
			...
			if($resc)
			{ 
				$insc2="INSERT INTO log_challanused SET
				ChallanNo			=	".$_REQUEST['ChallanNo'].",
				ActivityType		=	'INSERT',
				ActivityRefNo		=	'".$_REQUEST['ActivityRefNo']."',
				EmployeeId 			=	".$_SESSION['emp_id']."";
	...
	?>
	<?php
	$sql="SELECT * FROM challan_used WHERE ChallanNo=".$_REQUEST['ChallanNo']."";
	$res=mysql_query($sql, $conn2);
                ...
                <!--<a href="voucher_print.php?id=<?php //echo $_REQUEST['id'];?>">
					...
					<div class="widget_top">
						<span class="h_icon list_image"></span>
                     	<h6 style="float:center; font-size:16px;">Challan Status</h6>
					</div>
					<div class="widget_content">
						...
						<form action="" method="post" class="form_container left_label" onSubmit="return check_form();">
							<ul>
								...
								<li>
								<div class="form_grid_12">
									<label class="field_title">Challan Status<span class="req">*</span></label>
										<div class="form_input">
											<select name="IsUsed" id="IsUsed" data-required="required" data-message="Choose IsUsed Status" class="chzn-select custom-select" tabindex="2">
											<option value="">Select</option>
											<option value="0" <?php echo ($row['IsUsed']==0)?'selected':'';?>>UnUsed</option>
											<option value="1" <?php echo ($row['IsUsed']==1)?'selected':'';?>>Used</option>
											</select>
										</div>
								</div>
								</li>
