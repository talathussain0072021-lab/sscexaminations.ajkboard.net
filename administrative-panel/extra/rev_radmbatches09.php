...
<div id="container">
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action']=='update-record')
	{
		...
		if($_REQUEST['BatchStatus'] == 1)
		{
			...
			{
				$sql1="SELECT Id, BatchStatus, AdmStatus, RevStatus FROM vwadmbatches09 WHERE Id=".$BatchesArray[$i]."";
				...
				$sql2="UPDATE admbatches09 SET
				RevStatus			=		1,
				BatchStatus			=		".$BatchStatus."
				WHERE Id			=		".$row1['Id']."";
				$res2=mysql_query($sql2, $conn1);
				
				$sql3="UPDATE admbatchstudents09 SET
				RevStatus			=		1
				WHERE BatchId		=		".$row1['Id']."";
				$res3=mysql_query($sql3, $conn1);
			}//for($i=0; $i < sizeof($BatchesArray); $i++)
		
		?><script>alert('Batch Updated Successfully.');location.replace('rev_radmbatches09.php');</script><?php
		}//if($_REQUEST['BatchStatus'] == 1)
		else if($_REQUEST['BatchStatus'] == 2)
		{
			for($i=0; $i < sizeof($BatchesArray); $i++)
			{
				$sql1="SELECT Id FROM vwadmbatches09 WHERE Id=".$BatchesArray[$i]."";
				$res1=mysql_query($sql1, $conn1);
				$row1=mysql_fetch_array($res1);
					...
					<div class="widget_content">
						...
						<div class="invoice_action_bar" style="float: right;">
							<div class="btn_30_blue">
								<a href="rev_radmbatches_add09.php"><span class="icon add_co"></span>
						...
						{
							$SrNo=1; $P=0; $BatchFeeSum=0;
							$sql="SELECT Id, BatchNo, BatchStatus, AdmStatus, RevStatus, BatchCounter, InstituteId, BatchFee, ChallanNo FROM vwadmbatches09 WHERE BatchStatus=1 AND BatchType=1 AND RevStatus=0 AND ExamId=".$ExamId." AND InstituteId=".$_REQUEST['InstituteId']." ORDER BY Id ASC";
						...
						{
							$SrNo=1; $BatchSum=0;
							$sql="SELECT Id, BatchNo, BatchStatus, RegStatus, RevStatus, BatchCounter, InstituteId, BatchFee, ChallanNo FROM vwadmbatches09 WHERE BatchStatus=1 AND BatchType=1 AND RevStatus=0 AND SessionId=".$SessionId." ORDER BY Id ASC";
							$res=mysql_query($sql, $conn1);
							while($row=mysql_fetch_array($res))
							{?>
							<tr>
								...
								<td align="center"><?php echo floatval($row['BatchFee']);?></td>
								<td class="center"><span class="badge_style b_pending">Pending</span></td>
							</tr>
							<?php
							$SrNo++; $BatchSum+=$row['BatchFee'];
							}//while($row=mysql_fetch_array($res))
...
function update_batchstatus(form)
{
	...
	else
	{
		...
		if(BatchCount > 0)
		{
			location.replace('rev_radmbatches09.php?BatchesArray='+BatchesArray+'&BatchStatus='+BatchStatus+'&action=update-record');
		}
