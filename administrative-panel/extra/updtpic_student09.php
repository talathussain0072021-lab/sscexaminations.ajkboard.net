	...
	if(isset($_REQUEST['submit']))
	{
		if($_FILES['PicURL']['name'] != "" && $_REQUEST['newcheckbox'] == 1){
			...
			if($_REQUEST['IsRegular'] == 1)
			{
				...
				$serial_code=$_REQUEST['InstituteCode'].$RegY;
				$sql1="SELECT MAX(substring(PicURL,6,10)) AS MaxSerial FROM students WHERE substring(PicURL,6,10) like '".$serial_code."%'";
				...
				$target_path_image1 = 'pics/'.$serial_no.'.jpg';
				$target_path_image2 = '../institution-panel/'.'pics/'.$serial_no.'.jpg';
			...
			if($_REQUEST['IsRegular'] == 1)
			{
				...
				if((strpos($_REQUEST['PicURL1'], 'pics/') !== false) && ($int_length == 10))
				{
		...
		if(mysql_query($sql, $conn1))
		{
			...
			ActivityType			=		'PicInfoUpdation-I',
		...
		{
			?><script>alert('Error in Query.');location.replace('updtpic_student09.php?Id=<?php echo $_REQUEST['Id'];?>');</script><?php
		}
	}
	?>
	<?php
	$sql="SELECT Id, Name, FatherName, PicURL, Domicile, IsRegular, InstituteCode FROM vwregstudents WHERE Id=".$_REQUEST['Id']."";
			...
			<div class="grid_12 full_block">
				<div class="widget_wrap">
					...
					<div class="widget_content">
						...
						<input name="InstituteCode" id="InstituteCode" type="hidden" value="<?php echo $row['InstituteCode'];?>"/>
									...
									<ul>
										...
										<div class="form_grid_12">
											<div class="form_input">
												...
												<button type="button" name="reset" class="btn_small btn_blue" onclick="location.replace('updtpic_student09.php?Id=<?php echo $_REQUEST['Id'];?>')" tabindex="4"><span>Reset</span></button>
											</div>
										</div>
										</li>
									</ul>
