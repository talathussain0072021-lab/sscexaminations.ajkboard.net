...
<div id="container">
	...
	{
		$sql1="SELECT Id, BatchStatus, AdmStatus, RevStatus FROM vwadmbatches10 WHERE BatchNo='".$_REQUEST['BatchNo']."' AND BatchType=1 AND ExamId=".$ExamId."";
		...
		if($num_rows > 0)
		{
			...
			?><script>alert('Batch Updated Successfully.');location.replace('rev_radmbatches_add10.php');</script><?php
		}
		else
		{ ?><script>alert('Batch not Found.');location.replace('rev_radmbatches_add10.php');</script><?php }
	}//if(isset($_REQUEST['submit1']))
	
	if(isset($_REQUEST['submit2']))
	{
		$sql1="SELECT Id FROM vwadmbatches10 WHERE BatchNo='".$_REQUEST['BatchNo']."' AND BatchType=1 AND ExamId=".$ExamId."";
		$res1=mysql_query($sql1, $conn1);
		$row1=mysql_fetch_array($res1);
		$num_rows=mysql_num_rows($res1);
